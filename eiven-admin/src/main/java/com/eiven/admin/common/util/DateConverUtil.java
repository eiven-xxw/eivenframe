 ...
 *
 */
public final class DateConverUtil extends DateUtils {
	/**
	 ...
	 * @param sdate 传入的字符串。例如：2017-11-05
	 * @param toTypes 传入的字符串时间类型格式，例如：yyyy-MM-dd HH:mm:ss 
	 * @return Date日期对象
	 * 
	 *  示例：String time="2013-11-01 11:22";
	 *	 DateConverUtil.getDatebyStr(time);或者DateConverUtil.getDatebyStr(time,"yyyy-MM-dd HH:mm");
	 */
	public static Date getDatebyStr(String sdate, String... toTypes){
		Date date=null;
	 ...
	 */
	public static String getStrbyDate(Date date, String type){
		try {
	...
	public static String getSbySST(String sdate, String targetType, String... sourceType){
		try {
			return getStrbyDate(getDatebyStr(sdate, sourceType),targetType);
		} catch (Exception e) {
			return null;
		}
	}
